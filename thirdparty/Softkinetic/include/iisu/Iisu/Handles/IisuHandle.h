...
namespace SK
{
	...
	public:
		...
		/**
		 * \fn virtual SK::IisuHandle* shareHandle()=0;
		 * 
		 * \brief	Give a pointer to this handle that can be shared with an other thread/process
		 *
		 * \return	The shared handle pointer.
		 */
		virtual SK::IisuHandle* shareHandle()=0;

		/**
		 * \fn virtual bool isLocal()const=0;
		 * 
		 * \brief	Tells if the current handle is on a local iisu
		 *
		 * \return	true if the handle is open on a local iisu, false if it is a remote iisu.
		 */
