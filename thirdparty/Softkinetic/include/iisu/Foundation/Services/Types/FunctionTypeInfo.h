...
namespace SK
{
	...
	std::auto_ptr<TypelessInterface> FunctionTypeInfo::injectType(const SK::FunctionTypeInfo& typeInfo)
	{
		switch(typeInfo.typeID())
		{
			...
			SK_FUNC_TYPE_INJECT(SK::Return<void>(int32_t, const SK::Vector3&), FUNCTION_RETURNVOID_INT32T_VECTOR3);
			SK_FUNC_TYPE_INJECT(SK::Return<void>(int16_t), FUNCTION_RETURNVOID_INT16T);
