...
\
	...
	COMMAND_HELPER(DestroyController,			"UI.CONTROLLERS.Destroy",			SK::Result(int32_t));
	COMMAND_HELPER(CreateController,			"UI.CONTROLLERS.Create",			SK::Result(int32_t, const SK::Vector3&));
	...
	COMMAND_HELPER(StartRecording,				"SOURCE.RECORDING.Start",			SK::Result (const SK::String&));
	COMMAND_HELPER(StopRecording,				"SOURCE.RECORDING.Stop",			SK::Result (void));
